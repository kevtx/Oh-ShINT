...
class TestBaseProvider(unittest.TestCase):
    ...
    def test_try_load_token_from_env(self):
        ...
        with patch("os.getenv") as mock_getenv:
            ...
            # Should not load because no env var is set
            self.assertIsNone(provider.token)
    ...
    def test_try_load_token_with_correct_env_key(self):
        ...
        def mock_getenv_impl(key, default=None):
            if key == "CONCRETEPROVIDERFROM_API_KEY":
                return None
            if key == "CONCRETEPROVIDERFROM":
                return "token-from-env"
            return default

        with patch("os.getenv", side_effect=mock_getenv_impl):
            provider.try_load_token()
            # Should not load because class name will be ConcreteProvider (not CONCRETEPROVIDERFROM)
    ...
    def test_try_load_token_env_error_handling(self):
        ...
        with patch("os.getenv") as mock_getenv:
            mock_getenv.side_effect = Exception("File not found")
