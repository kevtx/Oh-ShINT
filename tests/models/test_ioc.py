...
class TestIOCRegexSearch(unittest.TestCase):
    ...
    def test_ioc_regex_search_ip(self):
        ...
        results = ioc_regex_search("ip", content)
        self.assertEqual(len(results), 2)
        self.assertIn("192.168.1.1", results)
        self.assertIn("8.8.8.8", results)

    def test_ioc_regex_search_ip_plural(self):
        """Test searching with plural form"""
        content = "IP: 192.168.1.1"
        results = ioc_regex_search("ips", content)
    ...
    def test_ioc_regex_search_domain(self):
        ...
        results = ioc_regex_search("domain", content)
        self.assertEqual(len(results), 2)
    ...
    def test_ioc_regex_search_sha256(self):
        ...
        results = ioc_regex_search("sha256", content)
        self.assertEqual(len(results), 1)
    ...
    def test_ioc_regex_search_sha1(self):
        ...
        results = ioc_regex_search("sha1", content)
        self.assertEqual(len(results), 1)
    ...
    def test_ioc_regex_search_md5(self):
        ...
        results = ioc_regex_search("md5", content)
        self.assertEqual(len(results), 1)
    ...
    def test_ioc_regex_search_no_matches(self):
        ...
        with self.assertRaises(ValueError) as context:
            ioc_regex_search("ip", content)
        self.assertIn("No matches found", str(context.exception))

    def test_ioc_regex_search_invalid_type(self):
        """Test searching with invalid regexp name"""
        content = "Some content"
        with self.assertRaises(ValueError) as context:
            ioc_regex_search("invalid_type", content)
        self.assertIn("Invalid regexp name", str(context.exception))
...
class TestGetIOCType(unittest.TestCase):
    ...
    def test_get_ioc_type_ipv4(self):
        """Test IPv4 detection"""
        ioc_type, category = get_ioc_type("8.8.8.8")
        self.assertEqual(ioc_type, "IPv4")
        self.assertEqual(category, "IP")

    def test_get_ioc_type_ipv6(self):
        """Test IPv6 detection"""
        ioc_type, category = get_ioc_type("2001:4860:4860::8888")
        self.assertEqual(ioc_type, "IPv6")
        self.assertEqual(category, "IP")

    def test_get_ioc_type_domain(self):
        """Test domain detection"""
        ioc_type, category = get_ioc_type("example.com")
        self.assertEqual(ioc_type, "Domain")
        self.assertEqual(category, "Domain")

    def test_get_ioc_type_md5(self):
        """Test MD5 detection"""
        ioc_type, category = get_ioc_type("d41d8cd98f00b204e9800998ecf8427e")
        self.assertEqual(ioc_type, "MD5")
        self.assertEqual(category, "MD5")

    def test_get_ioc_type_sha1(self):
        """Test SHA1 detection"""
        ioc_type, category = get_ioc_type("da39a3ee5e6b4b0d3255bfef95601890afd80709")
        self.assertEqual(ioc_type, "SHA1")
        self.assertEqual(category, "SHA1")

    def test_get_ioc_type_sha256(self):
        """Test SHA256 detection"""
        ioc_type, category = get_ioc_type(
            "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
        )
        self.assertEqual(ioc_type, "SHA256")
        self.assertEqual(category, "SHA256")

    def test_get_ioc_type_no_type_detected(self):
        """Test with no valid IOC type"""
        with self.assertRaises(ValueError) as context:
            get_ioc_type("not_an_ioc")
        self.assertIn("No IOC types detected", str(context.exception))
    ...
    def test_get_ioc_type_private_ip(self):
        ...
        ioc_type, category = get_ioc_type("192.168.1.1")
        self.assertEqual(ioc_type, "IPv4")
        self.assertEqual(category, "IP")
...
class TestIOCFactory(unittest.TestCase):
    ...
    def test_ioc_creates_domain(self):
        ...
        self.assertEqual(ioc.typ, "Domain")
    ...
    def test_ioc_creates_sha256(self):
        ...
        self.assertEqual(ioc.typ, "SHA256")
    ...
    def test_ioc_typ_property(self):
        """Test typ property returns class name"""
        ipv4 = IOC("8.8.8.8")
        self.assertEqual(ipv4.typ, "IPv4")

        domain = IOC("example.com")
        self.assertEqual(domain.typ, "Domain")
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_single_character_domain(self):
        """Test single character domains"""
        # Single char before dot should fail (needs at least one alpha)
        self.assertFalse(is_domain("1.2"))
        self.assertTrue(is_domain("a.b"))
